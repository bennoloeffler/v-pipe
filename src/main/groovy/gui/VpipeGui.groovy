...
import static gui.GridElement.*
import static gui.GridElement.nullElement
...
class GridPanel extends JPanel {
    ...
    GridPanel(int grid, GridModel model) {
        this.model = model
        this.grid = grid
        this.borderWidth = grid/2 as int
    }
...
class GuiMain {
    ...
    static void createFrame(GridPanel p) {
        JFrame frame = new JFrame("v-pipe")
        frame.getContentPane().add(new JScrollPane(p))
        frame.addKeyListener(p.listener)
